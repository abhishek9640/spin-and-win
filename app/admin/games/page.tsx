...
export default function GamesPage() {
  ...
  const getAuthToken = () => {
    // Since we don't know the exact structure of the session,
    // we'll check several common patterns
    if (!session) return '';
    
    // Try to access the token using different common patterns
    // @ts-expect-error - Checking multiple possible session structures
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch("https://spinwin.shreyanshkataria.com/api/v1/admin/create-game", {
        method: "POST",
        headers: {
